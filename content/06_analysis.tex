...
threedimensional spacetime via the DBSCAN algorithm (\autoref{subsec:dbscan}), rather than by setting
...
\begin{figure}[H]
  ...
  \caption{Twodimensional projections of the point cloud using the metric described above. The blue spheres represent single photons. Dense regions therefore appear in a darker blue.}
  \label{fig:pc}
\end{figure}
%
...
the cosmic gamma-rays by estimating the two features \texttt{disp} and the \texttt{sign}. The two
...
times per pixel, unlike the threedimensional space for DBSCAN, this cleaning can
